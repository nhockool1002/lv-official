...
function menuParentedit($data, $parent, $str='', $book){
    foreach($data as $item){
        if($item->parent_id == $parent){
            echo '<option value="'.$item->id;
            echo '"';
            if($item->id == $book->cat_id){
                echo "selected='selected'";
            }
            echo '>';
            echo  $str.$item->cat_name.'</option>';
            menuParentedit($data,$item->id,$str.'--', $book);
        }
    }
}
...
curl_setopt($ch, CURLOPT_POSTFIELDS, $voice);
...
$json = json_decode($result, true);
    echo $json['async'];
//echo '<audio src="'.$json['async'].'" hidden autoplay></audio>';
}
?>